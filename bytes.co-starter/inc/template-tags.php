...
if ( ! function_exists( 'starter_posted_on' ) ) {
	function starter_posted_on() {
		...
		$posted_on   = apply_filters(
			'starter_posted_on', sprintf(
				'<span class="posted-on">%1$s <a href="%2$s" rel="bookmark">%3$s</a></span>',
				esc_html_x( 'Posted on', 'post date', 'starter' ),
				esc_url( get_permalink() ),
				apply_filters( 'starter_posted_on_time', $time_string )
			)
		);
		$byline      = apply_filters(
			'starter_posted_by', sprintf(
				'<span class="byline"> %1$s<span class="author vcard"><a class="url fn n" href="%2$s"> %3$s</a></span></span>',
				$posted_on ? esc_html_x( 'by', 'post author', 'starter' ) : esc_html_x( 'Posted by', 'post author', 'starter' ),
	...
	function starter_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && starter_categorized_blog() ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %s', 'starter' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'starter' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %s', 'starter' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
